function pp = generateInitLyapunovJpl()
...
% data =  table2array(readtable('./data/Lyapunov_Jpl.csv'));
data =  table2array(readtable('./data/Vertical_Jpl.csv'));
mu  = data(end);
x0p = data(2:7)';
% x(:,1) = x0p;
% for j = 2:202
...
primary.T     = data(9);
%% Secondary
relState = [0; 0; 0; -0.2; 0.1; 0.1].*scale;
x0s = x0p + relState;        
...
secondary.C0 = [[3.7e-2 -5e-3 1.2e-3;
               -5e-3 2.8e-2 -6.2e-4;
               1.2e-3 -6.2e-4 4.3e-2] zeros(3,3); zeros(3,6)]*100;
