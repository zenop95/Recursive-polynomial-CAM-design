...
% for c = 1:n_constr
%     grad(c,:)      = DAArrays{c,1};                                             % [-] (n,1) Initialize grad with 1st-order 
%     pseudoH(:,:,c) = DAArrays{c,2}*(k>1);                                       % [-] (n,n) Initialize pseudoH with 2nd-order (if first-order set to zeros) 
%     for j = 3:k
%         pseudoH(:,:,c) = pseudoH(:,:,c) + fun_handles{j-2}(ctrl0,DAArrays{c,j});% [-] (n,n) Augmented pseudo-pseudoH with j-th order contribution
%     end
% end
grad      = nan(n_constr,n);
pseudoH   = nan(n,n,n_constr);
